...
export class GenerationService {
  ...
  async deck(workspaceId: string, reviewId: string) {
    const review = await this.prisma.review.findFirst({
      ...
      include: { account: true, template: { select: { name: true } }, workspace: { select: { name: true, accent: true } } },
    });
    if (!review) throw new NotFoundException('Review not found');
    return { html: this.buildDeck(review), name: review.templateName, account: review.account?.name ?? null };
  }
  ...
  async resolveShare(token: string) {
    const link = await this.prisma.shareLink.findUnique({
      where: { token },
      include: { review: { include: { account: true, workspace: { select: { name: true, accent: true } }, template: { select: { name: true } } } } },
    });
    if (!link) throw new NotFoundException('Link not found');
    if (link.expiresAt && link.expiresAt < new Date()) throw new BadRequestException('This link has expired');
    return this.buildDeck(link.review);
  }
...
<style>
  *{box-sizing:border-box} body{margin:0;font-family:'Instrument Sans',system-ui,sans-serif;background:#F4F6FB;color:#10152A}
  ...
  <div class="foot">Generated by MillionPulse AI · ${wsName} · Confidential</div>
</div></body></html>`;
  }
}
